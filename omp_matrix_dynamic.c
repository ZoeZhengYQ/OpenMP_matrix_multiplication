...
/* GLOBAL VARIABLE */
int next_work = 0;		// store the number of the row that should be allocated to next "worker"

int main(int argc, char const *argv[]) {
	int tid, nthreads, i, j, k;
	int MAT_SIZE, NUM_THREADS;
	double myC;					// store the result of each element in C
	double start_time, total_time;	// store runtime
	...
	#pragma omp parallel shared(nthreads, A, B, C) private(tid, i, j, k, myC)
	{
		// i: row iterator of A
		...
		#pragma omp master 
		{
			...
			printf("Starting parallel computing matrix multiplication with %d threads...\n", nthreads);
		}

		// here i specifies the number of the row that current thread need to calculate
		while (next_work < MAT_SIZE) {
			#pragma omp critical
			{
				i = next_work++;
				// printf("thread %d update next work to %d\n", tid, i+1);
			}
